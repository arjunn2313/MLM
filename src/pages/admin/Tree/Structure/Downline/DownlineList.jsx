...
import { useDownlineList } from "../../../../../hooks/useSection";
import SearchInput from "../../../../../components/Filter/Search";
import TablePlaceholder from "../../../../../components/loaders/TableSkelton";
import { allMemberList as downlineList  } from "../../../../../constatnts/TableHeadings";
...
export default function DownlineList() {
  ...
  const { data, isLoading, error } = useDownlineList(
    currentPage,
    searchQuery,
    treeName
  );
              ...
              <tbody className="block md:table-row-group capitalize">
                {data?.members?.map((data, index) => (
                  <tr key={index} className="border-t block md:table-row">
                    ...
                    <td className="p-2 block md:table-cell">
                      <Link
                        to={`?tab=DownlineMember&tree=downlinetree&memberId=${data?.memberId}&memberName=${data?.name}&sponsorId=${data?.sponsorId}&headName=${data?.sponsorName}`}
