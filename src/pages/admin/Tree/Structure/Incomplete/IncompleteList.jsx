...
import { useIncompleteList } from "../../../../../hooks/useSection";
...
export default function IncompleteList() {
  ...
  const { data, isLoading, error } = useIncompleteList(
    currentPage,
    searchQuery,
    treeName
  );
              ...
              <tbody className="block md:table-row-group capitalize">
                {data?.members?.map((data, index) => (
                  <tr key={index} className="border-t block md:table-row">
                    ...
                    <td className="p-2 block md:table-cell">
                      <Link
                        to={`?tab=IncompleteList&tree=incompletetree&memberId=${data?.memberId}&memberName=${data?.name}&sponsorId=${data?.sponsorId}&headName=${data?.sponsorName}`}
